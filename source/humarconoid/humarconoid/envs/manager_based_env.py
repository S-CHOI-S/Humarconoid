...
from humarconoid.managers import ARCActionManager
from isaaclab.managers import EventManager, ObservationManager, RecorderManager
...
class ARCManagerBasedEnv:
    ...
    * **Recorder Manager**: The recorder manager that handles recording data produced during different steps
    ...
    independently using the :attr:`ARCManagerBasedEnvCfg.decimation` (number of simulation steps per environment step)
    and the :attr:`ARCManagerBasedEnvCfg.sim.dt` (physics time-step) parameters. Based on these parameters, the
    ...
    def __init__(self, cfg: ARCManagerBasedEnvCfg):
    ...
    def load_managers(self):
        ...
        # -- action manager
        self.action_manager = ARCActionManager(self.cfg.actions, self)
        ...
        if self.__class__ == ARCManagerBasedEnv and "startup" in self.event_manager.available_modes:
    ...
    def step(self, action: torch.Tensor) -> tuple[VecEnvObs, dict]:
        ...
        time-steps can be configured independently using the :attr:`ARCManagerBasedEnvCfg.decimation` (number of
        simulation steps per environment step) and the :attr:`ARCManagerBasedEnvCfg.sim.dt` (physics time-step).
