...
from isaaclab.utils.math import quat_rotate_inverse, subtract_frame_transforms, yaw_quat
...
def heel_heeltoe_toe_seq(
    ...
    reward = torch.where(
        toe_contacted, torch.tensor(1.0, device=toe_contacted.device), torch.tensor(0.0, device=toe_contacted.device)
    )
...
def heel_toe_motion_air_time_positive_biped(
    ...
    command_name: str,
    threshold: float,
    heel_sensor_cfg1: SceneEntityCfg,
    toe_sensor_cfg1: SceneEntityCfg,
    heel_sensor_cfg2: SceneEntityCfg,
    toe_sensor_cfg2: SceneEntityCfg,
) -> torch.Tensor:
    ...
    right_heel_first = torch.all(heel_contact_time2 > toe_contact_time2, dim=1) & torch.all(
        toe_contact_time2 > 0, dim=1
    )
    ...
    if single_stance.any():  # single_stance가 하나라도 참인 환경이 있다면
        ...
        left_fully_contact = (
            (in_contact[:, 0].unsqueeze(-1) == 1) & (heel_contact_time1 > 0) & (toe_contact_time1 > 0)
        )  # tensor([[False, False], [False, False]], device='cuda:0')
        right_fully_contact = (
            (in_contact[:, 1].unsqueeze(-1) == 1) & (heel_contact_time2 > 0) & (toe_contact_time2 > 0)
        )  # tensor([[False, False], [False, False]], device='cuda:0')
        ...
        fully_contact = torch.where(
            torch.logical_or(left_fully_contact, right_fully_contact), 0.2, 0
        )  # torch.logical_or(left_fully_contact, right_fully_contact) # fully_contact: tensor([[False, False], [False, False]], device='cuda:0')
    ...
    single_stance_reward = torch.where(single_stance, 1, 0.0) * (
        command_norm > 0.1
    ...
    reward = (
        single_stance_reward
        + fully_contact_reward
        + single_stance_penalty
        + 0.25 * (left_heel_toe_sequence.float() + right_heel_toe_sequence.float())
    )  # 힐-토 모션 시 추가 보상
...
def leg_crossing_detection(
    ...
    reward = torch.where(
        single_support & position_sign_change,
        torch.tensor(1.0, device=relative_pos_x.device),
        torch.tensor(0.0, device=relative_pos_x.device),
    )
...
def get_gait_phase(env: ManagerBasedRLEnv, command_name: str, threshold: float = 0.3) -> torch.Tensor:
...
def ref_gait_phase(
    ...
    ref_left_joint_pos[torch.abs(sin_pos) < threshold] = 0.0
    ref_right_joint_pos[torch.abs(sin_pos) < threshold] = 0.0
    ...
    left_reward = torch.where(
        torch.all(ref_left_joint_pos == 0, dim=1), torch.tensor(0.0, device=left_reward.device), left_reward
    )
    right_reward = torch.where(
        torch.all(ref_right_joint_pos == 0, dim=1), torch.tensor(0.0, device=right_reward.device), right_reward
    )
...
def feet_contact_number(
    ...
    contact_status[left_valid_indices] += (
        left_feet_contact[left_valid_indices, 0] >= left_feet_contact[left_valid_indices, 1]
    )
    contact_status[right_valid_indices] += (
        right_feet_contact[right_valid_indices, 0] >= right_feet_contact[right_valid_indices, 1]
    )

    left_false_contact_indices = torch.where(
        (stance_mask[:, 0] == 0) & ((left_feet_contact[:, 0] != 0) | (left_feet_contact[:, 1] != 0))
    )[0]
    penalty[left_false_contact_indices] = -1.25

    right_false_contact_indices = torch.where(
        (stance_mask[:, 1] == 0) & ((right_feet_contact[:, 0] != 0) | (right_feet_contact[:, 1] != 0))
    )[0]
    penalty[right_false_contact_indices] = -1.25

    reward = contact_status.float() + penalty.float()
    reward *= torch.where(
        torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1) > 0.2,
        torch.tensor(1.0, device=stance_mask.device),
        torch.tensor(0.0, device=stance_mask.device),
    )
...
def feet_distance(
    ...
    reward[double_support_index] = torch.abs(
        left_foot_pos_b[double_support_index, 0] - right_foot_pos_b[double_support_index, 0]
    )
...
def foot_clearance(
    ...
    reward *= torch.where(
        torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1) > 0.2,
        torch.tensor(1.0, device=left_foot_height.device),
        torch.tensor(0.0, device=left_foot_height.device),
    )
